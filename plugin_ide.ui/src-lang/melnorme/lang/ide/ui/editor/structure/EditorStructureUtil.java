...
public class EditorStructureUtil {
	...
	public static void openInEditorAndReveal(Object selectedElement) throws CoreException {
		...
		if(selectedElement instanceof StructureElement) {
			...
			if(fileStructure == null || !fileStructure.getLocation(structureElement).isPresent()) {
				return;
			}
			
			IEditorInput newInput = EditorUtils.getBestEditorInputForLoc(fileStructure.getLocation(structureElement).get());
